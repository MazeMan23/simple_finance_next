...
export async function getStaticProps({ locale }) {
  const simpleProject = (await import(`../translations/project/${locale}.json`)).default;
  ...
  return (
    <Layout h={h} f={f}>
      ...
      <div className="flex flex-col items-center justify-center gap-4 w-11/12 mt-16 mx-auto lg:flex-row">
        <Image src="/images/pexels-andrea-logo.jpg" objectFit="cover" width={600} height={400} />
        <div className="flex flex-col mt-6 lg:ml-4 lg:mr-4 lg:w-1/2 gap-4">
          <div className="text-justify font-semibold indent-4 text-lg">{t("heroText")}</div>
        </div>
      </div>

      <h1 className="text-center text-3xl lg:text-4xl font-bold my-10">{t("our_works")}</h1>

      <div className="flex flex-col w-10/12 max-w-[595px] mx-auto mb-16 lg:w-11/12 lg:max-w-[1300px]">
        <div className="flex flex-col-reverse rounded-xl shadow-2xl mb-16 lg:flex-row ">
          <div className="mx-6 mb-6 self-center lg:ml-8 lg:w-[45%]">
            <h1 className="text-center text-2xl font-bold mb-4">{t("researchTitle")}</h1>
            <div className="text-justify indent-4 font-semibold">{t("researchText")}</div>
          </div>
          <div className="xl:mx-auto lg:my-8 lg:mr-6 flex">
            <Image src="/images/pexels-lukas.jpg" objectFit="cover" width={600} height={400} className="rounded-t-lg lg:rounded-lg" />
          </div>
        </div>
        <div className="flex flex-col rounded-xl shadow-2xl lg:flex-row">
          ...
          <div className="self-center mx-6 mb-6 lg:mr-8 lg:ml-6 lg:w-[60%]">
            ...
            <div className="text-justify indent-4 font-semibold">{t("projectManagementText3")}</div>
          </div>
        </div>
      </div>
